...
CREATE OR REPLACE TRIGGER TRG_RECYCLED_ITEM_DATE
BEFORE INSERT OR UPDATE ON RECYCLED_ITEM
FOR EACH ROW
BEGIN
    IF :NEW.RITEM_QC_DATE < SYSDATE THEN
        RAISE_APPLICATION_ERROR(-20001, 'RITEM_QC_DATE cannot be in the past');
    END IF;
END;
...
FOR EACH ROW
BEGIN
    IF :NEW.PAY_DATE < SYSDATE THEN
        RAISE_APPLICATION_ERROR(-20001, 'PAY_DATE cannot be in the past');
    END IF;
END;
